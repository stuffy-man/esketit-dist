...
ServerEvents.recipes((event) => {
  ...
  // ════ ASH BRICKS · Primary: supplementaries ════
  event.replaceInput(
    {},
    "the_deep_void:ash_bricks",
  ...
  // ════ CRATE DUPLICATES · Primary: farmersdelight/hearthandharvest ════
  // cratedelight adds generic crates that duplicate FD and H&H crates
  // Note: farmersdelight has NO apple_crate — that belongs only to the hhCrates list below
  const fdCrates = ["beetroot", "carrot", "potato"];
  fdCrates.forEach((item) => {
    event.replaceInput(
      {},
      `cratedelight:${item}_crate`,
      `farmersdelight:${item}_crate`,
    );
    event.replaceOutput(
      {},
      `cratedelight:${item}_crate`,
  ...
  // Note: cratedelight has no peanut_crate or sweet_berry_crate — those are H&H-unique
  const hhCrates = [
    "brown_mushroom",
    "red_mushroom",
    "egg",
  ...
  hhCrates.forEach((item) => {
    event.replaceInput(
      {},
      `cratedelight:${item}_crate`,
      `hearthandharvest:${item}_crate`,
    );
    event.replaceOutput(
      {},
      `cratedelight:${item}_crate`,
  ...
  // cratedelight gunpowder bag → H&H
  event.replaceInput(
    {},
  ...
  // Fruit crates already handled in unify_food.js
});
